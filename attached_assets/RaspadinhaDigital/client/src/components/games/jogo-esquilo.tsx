...
export default function JogoEsquilo() {
  ...
  // Audio cache refs to avoid reloading
  ...
  const playSound = (soundUrl: string, volume: number = 0.2) => {
    try {
      if (!audioCache.current[soundUrl]) {
        audioCache.current[soundUrl] = new Audio(soundUrl);
      }
      const audio = audioCache.current[soundUrl];
      audio.volume = volume;
    ...
    setTimeout(async () => {
      try {
        ...
        if (result.result === "fox") {
          ...
          // Always play chest open sound first
          playSound(chestOpenSound, 0.25);

          // Check if it's a bonus chest activation
          if (result.bonusActivated && !result.bonusUsed) {
            // Bonus was just activated - show chest selection
            ...
            // Update game state to bonus mode
            setGameState({
              ...gameState,
              gameStatus: "bonus",
                ...
                >
                  ...
                  >
                      ...
                      {gameState.useBonus ? (
                        <>
                          ...
                          <span className="font-bold text-yellow-400">
                            {bonusBalance} raspadinhas bônus
                          </span>
                          .
                        </>
                      ) : (
                    ...
                    <p className="text-white/60 text-sm mb-4">
                      {gameState.useBonus
                        ? "Ganhe mais raspadinhas bônus ou use saldo real."
