...
export default function Home() {
  ...
  const [gameSection, setGameSection] = useState<'raspadinhas' | 'minigames'>('raspadinhas');
  
  // Preload all game banners
  const gameBanners = [bannerPix, bannerMeMimei, bannerEletronicos, bannerSuperPremios, bannerEsquilo];
  ...
  // Check URL hash on mount to set correct game section
  useEffect(() => {
  ...
  const content = (
    <>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {/* Seção de Navegação Melhorada */}
        <section className="px-4 md:px-8 mb-6 md:mb-10 mt-8 md:mt-12">
          <div className="text-center mb-6 md:mb-8">
            {/* Título da Seção */}
            <h2 className="font-black text-[24px] md:text-[36px] tracking-[0.1em] uppercase mb-6">
            ...
            {/* Navegação em Tabs */}
            <div className="inline-flex items-center gap-0 p-1.5 bg-gray-900/80 backdrop-blur-sm rounded-2xl border border-gray-800 shadow-[0_4px_24px_rgba(0,0,0,0.4)]">
              {/* Tab Raspadinhas */}
              <button
                onClick={() => setGameSection('raspadinhas')}
                className={`relative px-6 py-3 md:px-10 md:py-4 rounded-xl font-bold text-sm md:text-base transition-all duration-300 flex items-center gap-2 md:gap-3 ${
                  gameSection === 'raspadinhas'
                    ? 'bg-gradient-to-r from-[#00E880] to-[#00FFB3] text-black shadow-[0_0_20px_rgba(0,232,128,0.5)]'
                    : 'text-gray-400 hover:text-white hover:bg-gray-800/50'
                }`}
              >
                <Grid3X3 className={`w-4 h-4 md:w-5 md:h-5 ${gameSection === 'raspadinhas' ? 'text-black' : 'text-gray-400'}`} />
              ...
              {/* Divider */}
              <div className="w-[2px] h-8 md:h-10 bg-gray-700/50 mx-1" />
              ...
              >
                <Gamepad2 className={`w-4 h-4 md:w-5 md:h-5 ${gameSection === 'minigames' ? 'text-black' : 'text-gray-400'}`} />
                <span className="uppercase tracking-wider font-black">
                  Minigames
                </span>
                <div className="absolute -top-2 -right-2 bg-orange-500 text-white text-[10px] px-2 py-0.5 rounded-full font-bold animate-pulse">
                  NEW
                </div>
              </button>
            </div>
