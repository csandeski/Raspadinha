...
export default function ParceirosDemo() {
  ...
  const { data: demoAccount, isLoading } = useQuery<DemoAccount | null>({
    queryKey: ['/api/partner/demo-account'],
    queryFn: async () => {
      try {
        const response = await fetch('/api/partner/demo-account', {
          headers: {
  ...
  const createMutation = useMutation({
    mutationFn: (data: typeof formData) => 
      apiRequest('/api/partner/demo-account', 'POST', data),
    onSuccess: () => {
      toast({
        title: "Conta demo criada!",
        description: "A conta foi criada com sucesso.",
      });
      queryClient.invalidateQueries({ queryKey: ['/api/partner/demo-account'] });
  ...
  const updateMutation = useMutation({
    mutationFn: (data: typeof formData) => 
      apiRequest('/api/partner/demo-account', 'PUT', data),
    onSuccess: () => {
      toast({
  ...
  const deleteMutation = useMutation({
    mutationFn: () => 
      apiRequest('/api/partner/demo-account', 'DELETE'),
    onSuccess: () => {
      toast({
      ...
      queryClient.setQueryData(['/api/partner/demo-account'], undefined);
      queryClient.invalidateQueries({ queryKey: ['/api/partner/demo-account'] });
      // Reset form data
      setFormData({ name: '', balance: 0, bonusBalance: 0 });
      setIsEditing(false);
  ...
  if (isLoading) {
    return (
      <PartnerLayout activeSection="demo">
