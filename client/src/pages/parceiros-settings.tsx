...
export default function ParceirosSettings() {
  ...
  // Fetch current settings - adapted for partner
  ...
  useEffect(() => {
    if (settingsData) {
      ...
      // Update profile data
      setProfileData({
        name: settingsData.name || localStorage.getItem('partnerName') || '',
        email: settingsData.email || localStorage.getItem('partnerEmail') || '',
  ...
  const updateProfileMutation = useMutation({
    mutationFn: async (data: typeof profileData) => {
      const token = localStorage.getItem('partnerToken');
      const response = await fetch('/api/partner/settings/personal', {
        method: 'PUT',
        headers: {
    ...
    onSuccess: async () => {
      // Update localStorage
      localStorage.setItem('partnerName', profileData.name);
      localStorage.setItem('partnerEmail', profileData.email);
      
      toast({
        title: "Perfil atualizado",
        description: "Suas informações foram atualizadas com sucesso"
      });
      await queryClient.invalidateQueries({ queryKey: ['/api/partner/settings'] });
      await queryClient.invalidateQueries({ queryKey: ['/api/partner/info'] });
  ...
  const updatePixMutation = useMutation({
    mutationFn: async (data: typeof pixData) => {
      const token = localStorage.getItem('partnerToken');
      const response = await fetch('/api/partner/settings/pix', {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || 'Failed to update PIX key');
      }
  ...
  const deletePixMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || 'Failed to delete PIX key');
      }
  ...
  const handleDeletePix = () => {
    if (confirm('Tem certeza que deseja remover sua chave PIX?')) {
  ...
  if (isLoading) {
    return (
      <PartnerLayout activeSection="settings">
              ...
              <div className="relative p-4 md:p-8">
                ...
                <div className="grid gap-4 md:gap-6">
                  ...
                  <Button
                    ...
                    className="w-full bg-gradient-to-r from-[#00E880] to-emerald-600 hover:from-[#00E880]/90 hover:to-emerald-600/90 text-black font-bold py-2.5 md:py-3 rounded-xl text-sm md:text-base"
                  >
                      ...
                      <div className="flex items-center gap-3">
                        {originalPixData.pixKey && (
                          <Button
                            ...
                            className="flex-1 bg-gray-800 hover:bg-gray-700 text-white font-bold rounded-xl py-2.5 md:py-3 text-sm md:text-base"
                          >
                            Cancelar
                          </Button>
                        )}
