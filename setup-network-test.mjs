...
async function setupNetworkTest() {
  ...
  try {
    ...
    // Update user 76 to have referred_by code TESTE2025
    const updateResult = await client.query(
      "UPDATE users SET referred_by = 'TESTE2025' WHERE id = 76"
    );
    
    console.log(`✅ Usuário ID 76 atualizado com código de indicação TESTE2025 (${updateResult.rowCount} linha atualizada)\n`);
    ...
    const userResult = await client.query(
      "SELECT id, name, referred_by, created_at FROM users WHERE id = 76"
    );
    ...
    if (userResult.rows.length > 0) {
      ...
      console.log(`- Código de indicação: ${user.referred_by}`);
