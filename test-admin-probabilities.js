...
async function testAdminProbabilities() {
  ...
  try {
    ...
    if (!gamesResponse.ok) {
      throw new Error(`Failed to get games: ${gamesResponse.status} - ${await gamesResponse.text()}`);
    }
    ...
    if (!probResponse.ok) {
      throw new Error(`Failed to get probabilities: ${probResponse.status}`);
    }
    ...
    if (!distributeResponse.ok) {
      throw new Error(`Failed to distribute equally: ${distributeResponse.status}`);
    }
    ...
    if (!resetResponse.ok) {
      throw new Error(`Failed to reset defaults: ${resetResponse.status}`);
    }
